  ...
  (test (parse `{cond {{= 0 1} {* 3 4}}
        ...
        (condE (list (pair (opE (eql) (numE 0) (numE 1))
                     ...
                     (pair (opE (eql) (numE 1) (numE 1))
                           (numE 8))))))
;; odcukrzanie
(define (Exp->CondlessExp [e : Exp]) : CondlessExp
  (type-case Exp e
    [(numE n) (numC n)]
    [(opE op l r) (opC op (Exp->CondlessExp l) (Exp->CondlessExp r))]
    [(ifE b l r) (ifC (Exp->CondlessExp b) (Exp->CondlessExp l) (Exp->CondlessExp r))]
    [(condE cs) (if (empty? cs)
                    (numC 42)
                    (ifC (Exp->CondlessExp (fst (first cs)))
                     (Exp->CondlessExp (snd (first cs)))
                     (Exp->CondlessExp (condE (rest cs)))))]))
...
(define (eval [e : CondlessExp]) : Value
  (type-case CondlessExp e
    [(numC n) (numV n)]
    [(opC o l r) ((op->proc o) (eval l) (eval r))]
    [(ifC b l r)
...
(define (main [e : S-Exp]) : Void
  (print-value (eval (Exp->CondlessExp (parse e)))))