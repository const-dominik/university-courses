...
(define (free-vars [e : Exp]) : (Listof Exp)
    (type-case Exp e
    [(opE o l r) (append (free-vars l) (free-vars r))]
    [(ifE b l r) (append (free-vars b) (append (free-vars l) (free-vars r)))]
    [(varE x) (list (varE x))]
    [(letE x e1 e2) (free-vars (subst e2 x e1))]
    [else (list)]))
...
(free-vars (parse `{let z 1
                {+ x {let x {+ y 1}
                       {* x 3}}}}))
(free-vars (parse `{if (= x 1) y z}))
(free-vars (parse `{let x 3 {let x 5 {+ x 2}}}))
(remove-duplicates (free-vars (parse `{if (= x x) x x})))
